...
SETUP_HINT = (
    "semantic operations require separately installed compatible owners in ANYfileio 0.3.2; "
    "install ANYmesher>=0.2,<0.6 and ANYmaterial>=0.1,<0.3"
)
...
@pytest.mark.parametrize(
    "versions",
    [
        {"ANYmesher": "0.1.9", "ANYmaterial": "0.1.0"},
        {"ANYmesher": "0.2.0rc1", "ANYmaterial": "0.1.0"},
        {"ANYmesher": "0.2.0+local", "ANYmaterial": "0.1.0"},
        {"ANYmesher": "0.6.0", "ANYmaterial": "0.2.0"},
        {"ANYmesher": "0.4.0", "ANYmaterial": "0.3.0"},
    ],
)
...
def test_module_or_symbol_failure_raises_sem003(monkeypatch: pytest.MonkeyPatch) -> None:
    _set_versions(monkeypatch, {"ANYmesher": "0.5.0", "ANYmaterial": "0.2.0"})
    modules = {
        "anymesher": SimpleNamespace(Mesh=DummyMesh),
...
def test_successes_are_cached_and_failures_are_not(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SemanticDependencyError):
        dependencies.require_semantics()
    versions.update({"ANYmesher": "0.5.0", "ANYmaterial": "0.2.0"})
...
@pytest.mark.parametrize("mesher_version", ["0.2.0", "0.4.0", "0.5.0"])
def test_supported_semantics_generations_are_accepted(
    monkeypatch: pytest.MonkeyPatch, mesher_version: str,
) -> None:
    _set_versions(monkeypatch, {"ANYmesher": mesher_version, "ANYmaterial": "0.2.0"})
...
def test_diagnostics_have_exact_codes_context_and_source_setup_hint(
    ...
    dependencies._reset_semantics_cache()
    _set_versions(monkeypatch, {"ANYmesher": "0.6.0", "ANYmaterial": "0.2.0"})
    with pytest.raises(SemanticDependencyError) as incompatible:
        dependencies.require_semantics()
    context = _assert_dependency_error(incompatible.value, "SEM002")
    assert context["required_range"] == ">=0.2,<0.6"
